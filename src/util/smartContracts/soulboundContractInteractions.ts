...
const soulboundContractBytecode = soulboundContractJson.bytecode;
const soulboundContractAddress = SOULBOUND_ADDRESS_HH as string;
...
const soulbound = new ethers.Contract(soulboundContractAddress, soulboundContractAbi, wallet);
...
export const createEventId = async (
  ...
  try {
    const createReceipt = await soulbound.createEventId(
      eventId,
      maxSupply,
      mintExpiration,
      eventOrganizer
...
export const getTokenEventId = async (tokenId: number) => {
  try {
    const txResponse = await soulbound.tokenEvent(tokenId);
...
export const getOwner = async () => {
  try {
    const txResponse = await soulbound.owner();
...
export const isAdmin = async (address: string) => {
  try {
    const txResponse = await soulbound.isAdmin(address);
...
export const mintToken = async (
  ...
  try {
    const txResponse = await soulbound.mintToken(eventId, to, initialData);
...
export const mintUserToManyEvents = async (
  ...
  try {
    const txResponse = await soulbound.mintToken(eventIds, to, initialData);
    console.log("mintToken Transaction response:", txResponse);
    return txResponse;
  } catch (error) {
    console.error("Failed to create event ID:", error);
...
export const ownerOf = async (tokenId: number) => {
  try {
    const txResponse = await soulbound.ownerOf(tokenId);
    console.log("mintToken Transaction response:", txResponse);
    return txResponse;
  } catch (error) {
    console.error("Failed to create event ID:", error);
